...
本次更新重点补全了设置页、词库管理和摸鱼模式，GitHub Actions 已自动构建 DMG：

- 💾 **数据管理补全**：清空错词本、清空收藏、重置全部进度、导出/导入学习数据
- 📚 **词库管理补全**：展示所有内置词库，自定义词库支持重命名、删除、导入预览和错误提示
- 🖥️ **真正开机启动**：接入 macOS 登录项 API，不再只是保存开关状态
- 🫥 **摸鱼模式增强**：透明度、小窗模式、固定右上角、隐藏/唤醒快捷键配置
- 🔧 **体验修复**：修复跟随系统主题点击不生效、学习进度溢出、自定义词库数据残留等问题
